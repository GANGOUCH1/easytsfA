...
class SparseRMoK(nn.Module):
    ...
    def __init__(self, hist_len, pred_len, var_num, num_experts=4, noisy_gating=True, k=4, drop=0.1, revin_affine=True):
        ...
        self.w_gate = nn.Parameter(torch.zeros(hist_len, num_experts), requires_grad=True)
        self.w_noise = nn.Parameter(torch.zeros(hist_len, num_experts), requires_grad=True)
    ...
    def cv_squared(self, x):
        ...
        return x.float().variable() / (x.float().mean() ** 2 + eps)
